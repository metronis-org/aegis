...
from metronis.db.models import EvaluationResultModel, TraceModel
...
class ElasticsearchService:
    ...
    def create_indices(self):
        ...
        if not self.client.indices.exists(index=self.evaluations_index):
            self.client.indices.create(
                index=self.evaluations_index, body=evaluations_mapping
            )
    ...
    def index_evaluation(self, evaluation: EvaluationResultModel):
        ...
        self.client.index(
            index=self.evaluations_index, id=str(evaluation.evaluation_id), document=doc
        )
    ...
    def search_traces(
        ...
        # Build query
        must_clauses = [{"term": {"organization_id": organization_id}}]
    ...
    def get_aggregations(self, organization_id: str) -> Dict:
        ...
        agg_body = {
            "query": {"term": {"organization_id": organization_id}},
            "aggs": {
                "by_domain": {"terms": {"field": "domain", "size": 10}},
                "by_model": {"terms": {"field": "model", "size": 10}},
                "traces_over_time": {
                    "date_histogram": {
                        "field": "created_at",
