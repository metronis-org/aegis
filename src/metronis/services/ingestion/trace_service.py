...
class TraceService:
    ...
    async def get_evaluation_result(
        self, trace_id: str, organization_id: UUID
    ...
    async def _validate_trace(self, trace: Trace) -> None:
        ...
        if len(trace.ai_processing.input) > max_input_size:
            raise ValidationError(
                f"Input size exceeds limit of {max_input_size} characters"
            )

        if len(trace.ai_processing.output) > max_output_size:
            raise ValidationError(
                f"Output size exceeds limit of {max_output_size} characters"
            )
